...
common::Error RedisInfo::SerializeFields(json_object* out) const {
  ...
  json_object_object_add(out, FIELD_REDIS_HOST_FIELD, json_object_new_string(host_str.c_str()));
  json_object_object_add(out, FIELD_REDIS_UNIX_SOCKET_FIELD, json_object_new_string(config_.redis_unix_socket.c_str()));
  return common::Error();
}
