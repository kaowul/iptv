...
void IBaseBuilder::HandleInputSrcPadCreated(common::uri::Url::scheme scheme, pad::Pad* pad, element_id_t id) {
  if (observer_) {
    observer_->OnInpudSrcPadCreated(scheme, pad, id);
  }
}

void IBaseBuilder::HandleOutputSinkPadCreated(common::uri::Url::scheme scheme, pad::Pad* pad, element_id_t id) {
  if (observer_) {
